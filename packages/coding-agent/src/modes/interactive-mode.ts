...
export class InteractiveMode implements InteractiveModeContext {
	...
	async #enterVibeMode(options?: { persistModeChange?: boolean; previousTools?: string[] }): Promise<void> {
		...
		this.showStatus(
			"Vibe mode enabled. You direct worker sessions (role: scout/utility/implementer/designer/planner/reviewer preferred when managed; cli fast/good fallback); toolset is read + optional parent Todo + vibe tools.",
		);
	}
	...
	setEditorComponent(
		...
		nextEditor.setTopBorderProvider(availableWidth => this.statusLine.getTopBorderRows(availableWidth));
