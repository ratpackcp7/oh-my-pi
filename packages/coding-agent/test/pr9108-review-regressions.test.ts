...
describe("PR #9108 review regressions", () => {
	...
	describe("routing snapshot reads only broker in-memory cache passively (no fetch, no SQLite probe)", () => {
		...
		it("warm broker cache populates matching account headroom", async () => {
			...
			const peek = vi
				.spyOn(auth as unknown as { peekBrokerModelUsageHealth: typeof auth.peekBrokerModelUsageHealth }, "peekBrokerModelUsageHealth")
				.mockReturnValue({
					state: "healthy",
					accounts: [
						{ credentialId: 1, credentialType: "oauth", accountKey: "other", state: "depleted", remainingFraction: 0 },
						{ credentialId: 2, credentialType: "oauth", accountKey: pool.accountKey, state: "healthy", remainingFraction: 0.82 },
					],
				});
		...
		it("intentional single-account nonmatching fallback preserves warm fraction", async () => {
			const auth = await AuthStorage.create(":memory:");
			await auth.set("anthropic", { type: "oauth", access: "a", refresh: "r", expires: Date.now() + 60_000, accountId: "candidate-account" });
			vi.spyOn(auth as unknown as { peekBrokerModelUsageHealth: typeof auth.peekBrokerModelUsageHealth }, "peekBrokerModelUsageHealth").mockReturnValue({
				state: "healthy",
				accounts: [{ credentialId: 1, credentialType: "oauth", accountKey: "different-account", state: "healthy", remainingFraction: 0.71 }],
			});
			const snapshot = await buildRoutingSnapshot(sessionWithRegistry(auth, registryFor(auth), routingOverrides));
		...
		it("ambiguous multi-account nonmatching stays unattributed (aggregate, no fraction)", async () => {
			...
			vi.spyOn(auth as unknown as { peekBrokerModelUsageHealth: typeof auth.peekBrokerModelUsageHealth }, "peekBrokerModelUsageHealth").mockReturnValue({
				state: "reserve",
				accounts: [
					{ credentialId: 1, credentialType: "oauth", accountKey: "acct-a", state: "healthy", remainingFraction: 0.9 },
					{ credentialId: 2, credentialType: "oauth", accountKey: "acct-b", state: "depleted", remainingFraction: 0 },
				],
			});
			const snapshot = await buildRoutingSnapshot(sessionWithRegistry(auth, registryFor(auth), routingOverrides));
