...
describe("StatusLineComponent scratch VCS boundary", () => {
	it.skipIf(process.platform === "win32")(
		"does not display a repository inherited from above the scratch root",
		() => {
			...
			const inheritedRepo = {
				...
				repoRoot: "/",
				isReftable: false,
			} satisfies VcsGitRepoInfo;
			const repoResolve = vi.spyOn(vcs, "gitInfo").mockReturnValue(inheritedRepo);
			// SuppressVcs must gate the whole VCS path: no branch/status lookups at all.
			const repoLookup = vi.spyOn(vcs, "repo");
