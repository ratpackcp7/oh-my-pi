...
export class Editor implements Component, Focusable {
	...
	#getVisibleContentHeight(contentLines: number): number {
		...
		// Hybrid: upstream style-aware verticalChrome plus fork's multi-row extra rows.
		const baseChrome = this.#effectiveStyle().verticalChrome;
		const extraRows = Math.max(0, this.#topBorderRowCount - 1);
		return Math.max(1, this.#maxHeight - (baseChrome + extraRows));
	}
	...
	render(width: number): readonly string[] {
		...
		// Hybrid: preserve fork's multi-row rendering on top of upstream's style dispatch.
		let topBorder: EditorTopBorder | undefined;
		let _hasPushedTop = false;
		if (style.statusAttachment !== "none" && borderVisible) {
			if (topBorders && topBorders.length > 0) {
				if (topBorders.length === 1) {
					topBorder = topBorders[0];
				} else {
				...
				// No status content: plain border will be rendered via style if needed
			}
		}
