...
- `role` selects the bundled agent via a tiny `VIBE_ROLE_AGENT` table; routing intent/pool handling reuses the native OMP task router (`task.routing.*` settings + `buildRoutingSnapshot` + `routeWorker`). A configured `task.routing.agentModels[agent]` roster limits that role's bundled agent to its curated candidate set while preserving dynamic scoring within the set. Concurrent Vibe workers in the same parent scope feed their in-flight resource-pool selections back into routing, so the router's normal sibling-diversity penalty applies across successive `vibe_spawn` calls instead of repeatedly selecting the same pool. No CP7 policy tables are copied into OMP.
