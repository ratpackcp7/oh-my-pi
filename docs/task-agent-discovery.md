...
Candidates are not the whole catalog. `buildRoutingSnapshot` starts from `modelRegistry.getAvailable()` filtered by `hasConfiguredAuth`. If `task.routing.agentModels[agent]` supplies a non-empty concrete roster, that roster is an **eligibility boundary** for the agent: unrelated models configured for other roles are not candidates, while the normal router still scores usage, cost, pool preference, parent anti-affinity, and sibling diversity within the roster. Without an agent-scoped roster, OMP keeps the existing global behavior and matches **intentional worker sources**: the explicit roster `task.routing.workerModels` and any concrete provider-qualified selector (`provider/model`) configured in `modelRoles` or `task.agentModelOverrides`. Bare aliases like `flash`, `mini`, `pro`, `haiku`, `codex` are **never** globally eligible — they would admit stale catalog models such as the former `google/gemini-1.5-flash` (404). When no intentional roster is configured, the fallback is the curated concrete priority roster (only provider-qualified selectors from `priority.json`: `slow`/`designer`/`smol` filtered to contain `/`), never bare aliases. Position in the selected list becomes the candidate's `preferredRank`, so configured ordering carries into scoring. Usage health comes from cached/last-good reports only — building a snapshot performs no network fetch and no credential refresh.
