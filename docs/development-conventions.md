# Development conventions — on demand

This file holds detailed repo-wide implementation rules that do not need to consume startup context on every Pi/OMP session. Read the relevant section before changing code in that area. Project orientation and task routers stay in the root `AGENTS.md`.

Catalog generation guidance is also documented in `packages/catalog/README.md` beside the generator it governs; use that package doc for current source locations. Rust profile settings remain authoritative in `Cargo.toml`, `.cargo/config.toml`, and `rust-toolchain.toml`; the historical rationale below is retained for decisions that are easy to accidentally re-litigate.
...
Workers re-enter the CLI entrypoint; never spawn separate worker entry modules. `cli.ts` declares itself as the worker host at startup (`declareWorkerHostEntry()` from `@oh-my-pi/pi-utils/env`) and dispatches hidden argv selectors (`__omp_worker_stats_sync`, `__omp_worker_tab`, `__omp_worker_js_eval`, `__omp_worker_tiny_inference`) before loading the command registry. Spawn sites use:
  ```ts
